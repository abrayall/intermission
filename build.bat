@echo off
REM Intermission Build Script for Windows
REM Packages the Intermission plugin as a WordPress-ready ZIP file

setlocal enabledelayedexpansion

echo ======================================
echo Intermission Build
echo ======================================
echo.
...
(
    echo # Intermission Version Information
...
REM Build Intermission Plugin
echo Building Intermission plugin...
set "PLUGIN_DIR=%BUILD_DIR%\intermission"
mkdir "%PLUGIN_DIR%"

REM Copy plugin files
copy /Y "intermission.php" "%PLUGIN_DIR%\" > nul
xcopy /E /I /Q "assets\*" "%PLUGIN_DIR%\assets\" > nul
xcopy /E /I /Q "templates\*" "%PLUGIN_DIR%\templates\" > nul
...
powershell -Command "(Get-Content '%PLUGIN_DIR%\intermission.php') -replace '\* Version: .*', '* Version: %VERSION%' | Set-Content '%PLUGIN_DIR%\intermission.php'"
...
cd /d "%BUILD_DIR%"
powershell -Command "Compress-Archive -Path 'intermission' -DestinationPath 'intermission-%VERSION%.zip' -Force"
echo [32m✓ Created: intermission-%VERSION%.zip[0m

REM Clean up temporary directory
cd /d "%SCRIPT_DIR%"
rmdir /s /q "%BUILD_DIR%\intermission"
